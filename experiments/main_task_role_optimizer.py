...
def role_hyperparameters(args: argparse.Namespace) -> dict[str, RoleHyperParameters]:
    ...
    # task_state_uniform（DualOptim 每场景独立状态）与 dual_optim_plus
    # 共享同一套统一角色学习率：唯一变量是二阶矩的分解方式。
    if args.optimizer_mode in ("task_state_uniform", "dual_optim_plus"):
...
def build_optimizer(
    ...
    if args.optimizer_mode == "dual_optim_plus":
        return DualOptimPlusOptimizer(registry, role_hyperparameters(args))
...
def run(args: argparse.Namespace) -> dict[str, object]:
    # 两端协作：默认 site B（原设计），site A 跑 baseline 时通过 LFM_SITE=A
    # 切换命名空间与 provenance。跨 site 的配对差仍然禁止，这里只放宽"谁可以
    # 启动"，不改隔离纪律（docs/20260817-1400 §2.1）。
    site = os.environ.get("LFM_SITE", "B").upper()
