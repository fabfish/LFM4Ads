...
- **F1 优化器状态共享度 baseline（探索，2 seed × 3.9%）**：DualOptim+（共享 base+残差）**系统性负向** −0.0167（2/2 同号，scaling up 省内存方案警示）；SkewAdam（角色分层）≈0（省内存不损性能）；DualOptim（每场景独立）方向不定。见 [`F1 结论`](../docs/20260818-1305-F1结论-优化器状态共享度baseline.md)。**踩坑**：site B 的 jacrev 梯度收集使全数据单 epoch ~2h 不可行，探索改用子采样。F2 EWC 已预注册待实现。
