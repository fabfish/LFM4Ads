...
Usage:
  python scripts/matrix/run_downstream_transfer_matrix.py plan
  python scripts/matrix/run_downstream_transfer_matrix.py execute
  python scripts/matrix/run_downstream_transfer_matrix.py status
"""
...
def ensure_source(model: str, seed: int) -> None:
    if source_done(model, seed):
        ...
        return
    run([PY, "scripts/matrix/run_downstream_source_pretrain.py",
...
def run_trial(arm: str, target: int, seed: int) -> None:
    if trial_done(arm, target, seed):
        ...
        return
    run([PY, "scripts/matrix/run_downstream_transfer_trial.py",
