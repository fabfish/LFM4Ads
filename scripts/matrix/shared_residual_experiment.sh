...
RUNNER="$REPO/scripts/matrix/run_shared_residual_experiment_matrix.py"
...
PY
      ...
      python scripts/verify/verify_shared_residual_experiment.py --write-report
    fi
    ;;
  start)
    exec 9>"$START_LOCK"
  ...
  status)
    ...
    ;;
  summarize)
    python scripts/summarize/summarize_shared_residual_experiment.py --write-results
    ;;
  stop)
    if ! is_running; then
