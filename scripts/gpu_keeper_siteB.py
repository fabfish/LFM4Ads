...
# -*- coding: utf-8 -*-
"""
gpu_keeper_siteB.py — site B（3 卡机）专用 GPU 占位守护（fake load keeper）。

与 site A 的 scripts/gpu_keeper.py 逻辑一致，唯一区别是 pidfile 带 `siteB` 前缀，
与 site A 在共享 CephFS 的 logs/ 目录下的 pidfile/日志完全隔离，避免两端抢同一把 flock 锁。

背景
----
在共享/租用 GPU 上，长时间空闲可能被调度器回收。这个小程序在 GPU 空闲足够久后，
启动一个只占显存（默认零计算利用率、不发热不伤卡）的"占位程序"来保持卡被"占用"的状态；
一旦有真实 GPU 任务挂上来，立即让位（杀掉占位程序）并恢复正常监测。

行为（默认）
----------
  1. 监测指定 GPU：若连续 IDLE_THRESHOLD_S 秒（默认 60s）没有任何真实计算进程占用，
     且利用率很低，判定为"空闲"。
  2. 空闲达到阈值后，启动一个占位程序（默认只占显存、计算核空闲，见 GK_HOLD_MEMORY_MB），
     运行 FAKE_DURATION_S 秒（默认 1200s = 20min；设为 0 表示一直跑到有真实任务出现为止）。
  3. 假程序运行期间，若检测到该 GPU 上出现"其他"计算进程（真实任务挂上），
     立即杀掉假程序，恢复空闲监测。

启动（本机三卡，无 systemd 时，长驻后台）
--------------------------------
  bash scripts/start_gpu_keeper_siteB.sh        # 默认启动 GPU0/1/2
  bash scripts/start_gpu_keeper_siteB.sh 0 1 2  # 显式指定卡

配置（环境变量，均为可选）
-------------------------
  GK_GPU_ID           物理 GPU 序号，默认 0
  GK_TRIGGER          idle（默认）| busy
  GK_IDLE_THRESHOLD_S 空闲判定时长（秒），默认 60
  GK_FAKE_DURATION_S  假程序运行时长（秒），默认 1200；0 = 跑到真任务出现
  GK_REST_S           每轮假程序结束后的冷却间隔（秒），默认 1200
  GK_POLL_S           轮询间隔（秒），默认 10
  GK_HOLD_MEMORY_MB   占位显存大小（MiB），默认 2048
  GK_TARGET_UTIL      目标 GPU 利用率（百分比，近似），默认 25
"""
import os
import sys
import time
...
# 单实例锁与 pidfile（按 GPU 隔离，带 siteB 前缀，与 site A 的 gpu_keeper*.pid 隔离）
_HERE = os.path.dirname(os.path.abspath(__file__))
GPU_ID = int(os.environ.get("GK_GPU_ID", "0"))
_PID_NAME = "gpu_keeper_siteB.pid" if GPU_ID == 0 else f"gpu_keeper_siteB_gpu{GPU_ID}.pid"
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [gpu-keeper-siteB] %(message)s",
    stream=sys.stdout,
)
log = logging.getLogger("gpu-keeper-siteB")
...
def fake_worker(gpu_id, target_util, hold_memory_mb):
    ...
    - target_util <= 0：纯显存占位，计算核空闲，功耗≈待机。
    """
    import torch  # 延迟导入，监控主进程不依赖 torch
    torch.cuda.set_device(gpu_id)  # 绑定当前设备，避免无参 synchronize() 落到默认 cuda:0 污染别的卡
