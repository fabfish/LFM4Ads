...
- **奥卡姆剃刀与短平快验证（用户 2026-08-17 更新，强制）**：这个小型研究仓库默认先跑最小可证伪实验，不得未经短测就扩成多随机种子或全训练集矩阵。探索阶段使用轻量模型（删除 `video_id/author_id/music_id` 三张高基数表）、单随机种子 42、固定训练批次数；核心口径对齐 27K 基线：五专家、每层保留两个专家、场景等权损失、学习率 0.001、每批 10,000、验证集八场景等权指标。**待验证方向不得缩水**：状态语义、专家学习率、路由器学习率和共享主干学习率的所有预注册候选都要保留，但去掉重复配置后均匀分配到三张同型号显卡同时运行，以最快得到方向性读数。跨显卡短测只作方向筛选，不能下正式配对结论；只有观察到值得复核的差异后，才单独授权增加随机种子或训练预算。
