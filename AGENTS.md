...
- **每轮对话结束必须 git commit 并 push（强制，用户 2026-08-17 两次重申）**：agent 在**每一轮对话结束前**，无论本轮是否完成一个完整逻辑单元，都必须 `git add` 相关文件 → `git commit` → `git push origin main`，**确保工作区干净（无未提交改动）**。这是一个动作闭环，不是可选项；**不等用户提醒、不等用户授权**。
  - **触发时机（本轮新增）**：以「一轮对话」为硬性边界，**每轮对话结束前兜底提交一次**，保证任何时刻 `git status` 干净、另一端（site B）随时能 pull 到最新一致状态。即使本轮只是查询/答疑、未产生代码或文档改动，也须在结束时确认工作区无遗留未提交文件（若有则提交）。
  - **提交粒度**：一轮对话内每完成一个逻辑单元（代码改动 / 实验 run / 文档更新）commit 一次，不要攒多个无关改动混在一个 commit 里。
  - **commit message 规范**：写清「哪个实验 / 改了什么 / 什么变量 / 什么判定」，便于两端追溯。
