...
| **E17 s0 baseline + AdaTask win-case** | `planned` | **NOT_EVALUATED** | s0 在 E10 为 MoE−dense `+0.007529`（4/4 正）；F1 短训纯 task-state−shared 为 `+0.020936/+0.041955`，仅作预选依据 | 先做 two-group 梯度与 task-axis post-direction 审计；seed101 全数据 gate 后才可扩 4 seed | [复审与预注册](./20260818-1815-结论复审与E17单场景AdaTask-win-case预注册.md) |
