...
| 下游留出域参数高效迁移 | `running` | `NOT_EVALUATED`（G1 进行中） | G0 技术+G0-final 审计均 PASS、`formal_training_authorized=true`；现有 MoE 下游仅有 FeatureUsage 探索，所谓 Module 行是不加载 backbone 的随机初始化占位；检验 4096 总标注下 router-only 对下游可训练参数近似匹配 dense adapter 的优势 | `auth=authorized`（2026-08-12 用户「host 该实验开始执行」）；GPU 矩阵按 G1→G2 执行 | [新驱动](archive/drivers/20260812-2303-MoE下游留出域参数高效迁移驱动.md) |
...
- 当前证据：G0 全过；GPU 矩阵由 `scripts/run_downstream_transfer_matrix.py` 执行中（G1=seed 42 验证门控先行，通过才续 seeds 123/456）。尚无 trial 结果、manifest 或优势判定；优势状态仍 `NOT_EVALUATED` 直至 G2 门控冻结。
- 启动边界：G0 与 authorization 已满足，正式 GPU 矩阵已按阶段门控启动；任何预算/参数/数据隔离改动需重新冻结 authorization。
...
新下游路线已 `authorized / running`：按[正式驱动](archive/drivers/20260812-2303-MoE下游留出域参数高效迁移驱动.md)由 `scripts/run_downstream_transfer_matrix.py` 执行，G1（seed 42 验证门控）先行，通过才续 seeds 123/456；G2 应用预注册门控冻结 verdict。
