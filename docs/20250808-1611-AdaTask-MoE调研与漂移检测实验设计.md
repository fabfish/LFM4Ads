...
> **数据核查**: 本文所有数字均可由 `python scripts/verify_provenance.py` 复现，偏差记录见
...
`python scripts/extract_downstream.py` 从日志的 `Method` 字段还原）：
...
| `scripts/verify_provenance.py` | 根目录 | **数据溯源核查**：C1-C10 十项检查 |
| `scripts/extract_downstream.py` | 根目录 | 从日志还原完整下游评估（CSV 未跑完时的权威来源） |
...
> 无法支撑本节任何关于场景 5/8 的结论。提取命令：`python scripts/extract_downstream.py`。
...
| `cache/provenance_report.json` | `scripts/verify_provenance.py` 的机读核查结果 | ✅ 每次运行重新生成 |
...
**独立运行（不与 Phase1 混用）**：`scripts/run_phase3_spec.py` 从 `cache/dcnv2_vanilla.pt`
...
- [x] 数据溯源核查脚本 `scripts/verify_provenance.py`
